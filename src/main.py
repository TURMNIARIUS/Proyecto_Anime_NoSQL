"""Modulo principal de la API.

Provee endpoints REST con FastAPI para búsqueda, analítica y motores
de recomendación (colaborativo y por contenido). Incluye utilidades
de preprocesamiento y control de memoria para entornos con recursos
limitados.
"""
...
class ConsultaContenido(BaseModel):
    """Esquema de entrada para la búsqueda por contenido.

    Atributos:
        generos: lista de géneros requeridos (lista de strings).
        anio_min: año mínimo de emisión (inclusive).
        anio_max: año máximo de emisión (inclusive).
        descripcion: texto en español que describe la trama buscada.
    """
    generos: list[str]
    anio_min: int
    anio_max: int
...
def eliminar_duplicados(lista_animes, limite=None):
    """Filtra duplicados por título en una lista de diccionarios.

    Args:
        lista_animes: lista de diccionarios que representan animes.
        limite: opcional, número máximo de animes únicos a devolver.

    Returns:
        Lista de diccionarios sin títulos repetidos (orden original preservado).
    """
    vistos = set()
    unicos = []
...
def read_root():
    ...
    Retorna un JSON con estado de la API para comprobaciones de salud.
    """
...
def buscar_anime(titulo: str):
    """Buscar animes por título (búsqueda difusa, case-insensitive).

    Realiza una consulta con expresión regular y devuelve hasta 15
    resultados únicos ordenados por puntuación.
    """
    regex = re.compile(f".*{titulo}.*", re.IGNORECASE)

    # .find() devuelve un cursor, lo ordenamos por score descendente y limitamos a 50
    # Limpiamos duplicados para que nos queden al menos 15 únicos
...
def top_generos_calificados():
    """Calcula los géneros mejor calificados mediante un aggregation pipeline.

    Devuelve los 10 géneros con mayor promedio de puntuación, filtrando
    aquellos con menos de 50 títulos para asegurar significancia.
    """
...
def top_por_genero(genero: str):
    """Devuelve los 5 mejores animes de un género dado.

    El método realiza una búsqueda con regex para capturar coincidencias
    parciales y luego elimina duplicados para garantizar diversidad.
    """
...
def obtener_usuarios_aleatorios():
    """Recupera perfiles aleatorios desde la colección de reseñas.

    Utiliza `$sample` para eficiencia y devuelve hasta 5 perfiles.
    """
...
def recomendacion_colaborativa(profile: str):
    """Genera recomendaciones mediante filtrado colaborativo.

    Pasos resumidos:
    1. Recupera el historial del perfil objetivo.
    2. Recupera vecinos con intersección de items.
    3. Construye matriz usuario-item y calcula similitud coseno.
    4. Devuelve top-5 animes recomendados.
    """
...
def recomendacion_por_contenido(consulta: ConsultaContenido):
    """Genera recomendaciones por contenido usando TF-IDF.

    El flujo incluye filtrado por género/año en MongoDB, traducción
    del prompt (deep-translator), vectorización TF-IDF y cálculo de
    similitud coseno. Se aplican límites de documento y vocabulario
